...
| 签退 (signback) | 时段结束时自动签退，由下一个守护账号接力下一段。⚠️ 真签退端点是 `POST /data/apps/seat/signback`；`/leave` 是**暂离**不是签退（2026-08-25 修正） |
...
| **签退窗口** | `signback` 时段进行中任意时刻可用；`/leave`（暂离）要求剩余 ≥20min。2026-08-25 实测修正（原文"时段结束前 20min 可签退"是把暂离规则误当签退）。 |
