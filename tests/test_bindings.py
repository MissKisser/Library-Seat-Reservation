...
def _seat_target(desired=None) -> SeatTarget:
    return SeatTarget(seat_num="030", desired_slots=desired)


# ---------- matrix_windows ----------

def test_matrix_windows_weekday_dict_only_that_day():
    m = {"030": {"mon": ["09:00-11:00"], "tue": ["15:00-17:00"]}}
    assert [w[0] for w in matrix_windows(m, "mon")] == ["030"]
...
def test_validate_matrix_two_slots_different_days_ok():
    ok = {"030": {"mon": ["09:00-11:00"], "tue": ["09:00-11:00", ]}}
    validate_matrix(ok, max_seg_hours=2.0, daily_limit_hours=5.0)
...
def test_validate_matrix_daily_limit_per_weekday():
    ok = {"030": {"mon": ["08:00-10:00"]}, "031": {"mon": ["10:00-12:00"]},
           "032": {"mon": ["13:00-15:00"]}}
...
def test_validate_matrix_overlap_only_same_day():
    ok = {"030": {"mon": ["09:00-11:00"]}, "031": {"tue": ["10:30-12:30"]}}
    validate_matrix(ok, max_seg_hours=2.0, daily_limit_hours=5.0)
    bad = {"030": {"mon": ["09:00-11:00"]}, "031": {"mon": ["10:30-12:30"]}}
...
def test_account_margins_shape():
    accs = [_acc("a", {"030": {"mon": ["09:00-11:00"]}})]
...
def test_candidate_accounts_checks_only_given_weekday():
    accs = [
        _acc("busy_mon", {"030": {"mon": ["09:00-11:00"]}}),
        _acc("free_tue", {"031": {"tue": ["09:00-11:00"]}}),
    ]
    from datetime import time as _t
    out = candidate_accounts(
        accs, seat="030", start=_t(9, 0), end=_t(11, 0),
...
def test_auto_assign_respects_per_day_overlap():
    accs = [_acc("a", {"030": {"mon": ["09:00-11:00"]}})]
    desired = {"031": {"mon": ["10:00-12:00"], "tue": ["10:00-12:00"]}}
    matrices, unfillable = auto_assign(
        accs, desired, max_seg_hours=2.0, daily_limit_hours=5.0)
    assert matrices["a"]["031"].get("tue") == ["10:00-12:00"]
    assert matrices["a"]["031"].get("mon") in (None, [])
...
def test_auto_assign_keeps_existing_bindings():
    ...
    matrices, _ = auto_assign(
        accs, desired, max_seg_hours=2.0, daily_limit_hours=5.0)
    assert matrices["a"]["030"]["mon"] == ["09:00-11:00"]
    assert matrices["a"]["030"]["tue"] == ["15:00-17:00"]


def test_auto_assign_output_is_canonical_seven_keys():
    accs = [_acc("a", {})]
    desired = {"030": {"mon": ["09:00-11:00"]}}
    matrices, _ = auto_assign(accs, desired, max_seg_hours=2.0, daily_limit_hours=5.0)
    assert set(matrices["a"]["030"].keys()) == {
